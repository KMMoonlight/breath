...
- [ ] 构建产物支持 arm64 与 x86_64，最低系统为 macOS 14
- [ ] 发布产物通过 Developer ID 签名与 Apple 公证且不启用 App Sandbox
- [ ] GitHub Releases 托管安装与更新包，GitHub Pages 托管签名 appcast
- [ ] Sparkle 自动检查但只在用户确认后安装，取消不影响进程
- [ ] 安装重启复用正常退出流程并遵守最后选择的按需恢复策略
- [ ] 更新同时验证 Apple Code Signing 与 EdDSA，不需要 GitHub Token 或 Breath 后端
