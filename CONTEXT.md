...
当前输入焦点位于终端窗格时，可能与 Breath 自有快捷键冲突的按键先交给终端及其中运行的 Agent CLI；终端返回未处理时，同一个按键继续交给 Breath。输入焦点不在终端时由 Breath 直接处理，窗格级动作指向最后聚焦的终端窗格。新建工作会话与按编号选择工作会话 Tab 属于应用级导航命令，不进入终端优先仲裁；`⌘[` 和 `⌘]` 分别循环聚焦当前工作会话中的上一个和下一个分屏。macOS 系统生命周期快捷键不属于此规则。
