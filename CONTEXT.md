...
**活动栏（Activity Bar）**：
主窗口最左侧固定的图标导航；“工作区、笔记、自动化、Git、Skills、额度”从顶部依次排列，“设置”固定在活动栏底部。只有选择“工作区”时才展示会话树与终端分栏；选择其他入口时隐藏会话树，并让对应页面占满活动栏右侧区域。自动化入口可以显示尚未查看的成功、失败、超时和中断结果数量，超过 99 时显示 `99+`。
_Avoid_: 侧栏底栏、状态栏、工具栏

**笔记库（Note Library）**：
用户选择、全局唯一的真实本地目录，是所有 Note Document 的文件系统边界。笔记库独立于工作区，不出现在 Workspace 列表、会话树或 Git 工作台中；目录不可用时 Breath 保留引用并提供重试或更换目录，不自动重建、移动或删除目录。
_Avoid_: 笔记工作区、知识库工作区、项目笔记

**笔记文档（Note Document）**：
笔记库内由 Notes 功能打开的真实文件。Markdown 和文本文件的磁盘内容是唯一事实来源；Breath 可以保存界面元数据、可重建索引和崩溃恢复快照，但这些数据不取代源文件。
_Avoid_: 数据库笔记、富文本文档、云笔记

**笔记标签（Note Tab）**：
一个笔记文档在 Breath 中的打开和编辑状态，包含内存缓冲区、最后保存基线、脏状态、冲突和恢复信息。同一规范化文件最多对应一个笔记标签；它不属于工作会话标签栏。
_Avoid_: 工作会话 Tab、预览标签、编辑器分组

**自动化（Automation）**：
用户保存、绑定到一个现有工作区的固定 Prompt、执行 Agent 和触发规则。自动化独立于工作会话，创建和运行都不会创建工作会话、终端窗格、Agent 对话、Worktree 或 Git 分支；名称可以重复，内部稳定 ID 不展示。自动化按创建时间从新到旧排列，创建后默认启用但不会立即运行。
_Avoid_: 任务、工作会话、Agent 对话、后台守护进程

**自动化运行（Automation Run）**：
一次由手动、计划或外部命令触发的无头 Agent 执行。它直接以真实工作区作为只读工作目录，在本次运行专属的临时 HOME 和 macOS 进程沙盒中运行，只持久化最终回答、清理后的短错误和最小元数据；每个自动化最多保留最近五次。运行状态只取“已排队、运行中、成功、失败、已超时、已取消、已中断、已跳过、已错过”之一。
_Avoid_: 任务执行、Agent 回合、终端进程、工作区副本

**自动化触发规则（Automation Trigger）**：
决定自动化何时进入统一运行队列的本地规则，只取“手动、单次、每天、每周、固定间隔、五字段 Cron、外部命令”之一。计划只在 Breath 进程运行时生效；完全退出或用户主动睡眠期间不补跑，错过的 occurrence 聚合为一条“已错过”记录。
_Avoid_: 云端调度、launchd 任务、每月专用规则、自动重试

**自动化短码（Automation Shortcode）**：
外部命令触发自动化时由 Breath 生成的稳定不透明短标识。用户通过 `breath trigger <短码>` 向正在运行的 Breath 当前用户 Unix Domain Socket 发送无 payload 的触发请求；短码重新生成或自动化删除后旧值立即失效。
_Avoid_: Webhook URL、HTTP 服务、动态 Prompt、远程触发令牌
...
**笔记 Agent 对话（Note Agent Conversation）**：
从 Notes 抽屉启动、以笔记库为工作目录的单一 Agent 对话。它拥有 Agent 类型和官方 Agent Session ID，但不属于工作区、工作会话或终端窗格；Breath 不保存其提示词、回复、工具输入输出或转录。
_Avoid_: 笔记工作会话、笔记终端窗格、内置 AI 聊天

**笔记 Agent 终端（Note Agent Terminal）**：
承载笔记 Agent 对话的真实原生终端运行时。隐藏抽屉、切换活动栏页面或关闭窗口不会停止它；只有用户明确结束对话或完全退出 Breath 才停止。它使用终端配置，不使用 Markdown 主题。
_Avoid_: Terminal Pane、Web 聊天、Workspace 终端
...
**额度检查（Quota Check）**：
对当前已安装且受支持的 Agent CLI 的 Agent 额度及其可用状态的汇总；未安装的 Agent 不进入汇总，也不创建卡片。每个 Agent 独立产生结果，一个 Agent 失败不使其他结果失效。每个 Agent 使用固定宽高的独立卡片，页面按可用空间横向排列并自动换行，不把卡片拉伸为整行；卡片内的额度窗口也使用统一的固定宽高。
_Avoid_: 全部受支持 Agent 清单、Agent 排行、用量估算

**额度检查状态（Quota Check Status）**：
单个已安装 Agent 在一次额度检查中的结果分类，只能是“查询中、可用、未登录、不支持查询、查询失败”之一。界面不额外展示“可用、不支持查询、查询失败”等状态标签，而是在对应品牌 Logo 旁通过 Agent 名称颜色表达：可用为绿色，未登录或不支持查询为灰色，查询失败为橙色；查询中保持中性灰色。“查询失败”仍可在卡片内容区附带经过脱敏的简洁原因，但不得包含令牌、账号标识或其他凭据信息。
