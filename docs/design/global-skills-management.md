...
一个全局 Skill 可以在多个 Agent 中分别拥有安装副本。列表使用 `name + 规范化内容摘要` 聚合内容一致的副本，并在同一行展示多个 Agent；名称相同但内容不同的 Skill 是不同条目，通过 `description`、来源和路径自然区分，不增加“同名冲突”标签。
...
- 在侧栏底部栏右侧、紧邻“任务视图”增加 Skills 按钮。
- Skills 是与任务视图同级的全局主内容模式，不依赖当前工作区或工作会话。
- 无工作区时仍可进入。
- 选中按钮沿用任务按钮的选中态。
- 用户选择任一工作会话后返回会话终端内容区。
...
- 完整 `description` 与可展开的 `SKILL.md`。
- 文件清单、真实路径和“在 Finder 中显示”。
- 每个 Agent 副本的来源、上游、Commit、内容状态和存储形式。
- 更新与卸载操作。
...
- 搜索结果不得以整行点击或无文字箭头触发远程下载。未发现本地同名 Skill 时，每行提供明确的“安装”按钮；发现同名 Skill 时，展示它已安装到的 Agent，并将操作标记为“检查安装…”。用户点击后解析实际安装来源，再进入与 GitHub 相同的候选和预览流程。
- 下载前只能根据 Skill 名称标记本地同名安装，不能声称内容相同。内容是否相同、是否需要覆盖，必须在下载并计算内容摘要后的安装预览中确定。
...
1. 侧栏底部栏右侧出现 Skills 按钮，无工作区时仍可进入；选择工作会话后返回终端。
