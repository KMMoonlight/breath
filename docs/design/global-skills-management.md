...
- `~/.agents/skills` 中的共享副本不显示逐 Agent 复选框；确认界面按解析后的物理目录聚合，展示共享真实路径和全部受影响 Agent（包括通过符号链接发现它的 Agent）。用户确认后先移除所有已验证仍指向该目录的 Agent 符号链接，再只将该物理目录移入废纸篓一次；链接移除或移入废纸篓失败时恢复已移除的链接，避免留下悬空链接。若 `~/.agents/skills` 中的条目本身是符号链接，则聚合共同使用该发现链接的 Agent（包括直接链接到该发现链接的 Agent）并移除这些链接，不删除其外部目标。
