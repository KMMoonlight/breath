...
主窗口只有一个，最左侧固定一列活动栏。“工作区、任务、Git、Skills”从顶部依次排列，“设置”固定在活动栏底部。选择“工作区”时，活动栏右侧采用会话树与终端区域的分栏：
...
终端区域
├── 工作会话 Tab · 工作会话 Tab · 新建（随 Tab 滚动） | 编辑器（固定右侧）
└── 当前工作会话的递归分屏布局，无底栏
```

- 同一规范化目录只能添加为一个工作区。
- 一个工作区可包含多个工作会话。
- 当前工作区的全部活跃工作会话在终端区域顶部以浏览器式 Tab 并列展示；新建按钮紧跟最后一个 Tab 并随 Tab 一起横向滚动，编辑器入口固定在标签栏右侧；Tab 横向溢出时滚动。
...
- `⌘W` 在多窗格时关闭当前窗格；仅剩一个窗格时直接归档当前工作会话，不关闭 Breath 主窗口。
...
设置页包含独立“网络”Tab。代理方式只有“不使用代理”“使用系统代理”“使用手动代理”三种；手动代理提供 URL、可选用户名和可选密码，密码只写入 macOS Keychain。页面下方提供默认地址为 `https://www.google.com` 的连接测试。代理适用于 Breath 内置网络请求和 Breath 发起的 Git HTTP(S) 操作，但不改变终端命令、Sparkle 应用更新或 macOS 系统代理设置；这一范围通过“网络代理”标题旁的说明图标展示。
...
Breath 的应用配置和终端配置都只负责样式：

- **应用配置**：只负责终端之外应用界面的样式。
- **终端配置**：只负责终端内部样式，例如字体、字号、颜色主题和光标。

Shell、快捷键、分屏和 Agent 行为不属于这两类样式配置。快捷键页另行保存终端焦点下的所有权策略，但 Breath 不读取、导入或同步 Ghostty 配置。

快捷键按当前输入焦点和用户选择的所有权策略仲裁。焦点位于终端窗格时，未被 Breath 注册为宿主快捷键的按键始终交给窗格内运行的 Shell、Agent CLI 或其他 TUI，例如 `Shift+Tab` 由 Claude Code 等终端内应用处理。“Breath 优先”是默认策略，已注册的应用级快捷键由 Breath 处理，例如 `⌘T` 新建工作会话；“终端优先”把应用级快捷键交给终端内应用，不在子进程无可见响应时回退 Breath。终端级 Breath 快捷键在两种策略下均由 Breath 处理，包括分屏、关闭分屏和分屏焦点切换。Ghostty 自身的窗口、Tab、分屏等宿主级动作不参与此优先级，Breath 生成内部 Ghostty 配置时取消与自身快捷键冲突的内建绑定。焦点不在终端时由 Breath 直接处理；窗格级快捷键作用于当前或最后聚焦的终端窗格。

PTY 键盘协议不提供“应用内部是否绑定了当前组合键”的逐键回执；成功编码并送入子进程只能证明按键可传输，不能证明终端内应用会处理它。因此两种策略都在发送按键前静态决定所有权，不尝试逐键动态回退。
