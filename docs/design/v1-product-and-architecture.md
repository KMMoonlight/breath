...
- 自动化、普通聊天、Computer Use、Git worktree 等后续能力。
- Breath 账户、后端、云同步或遥测。
...
- 已归档工作会话不出现在会话树，只能在设置窗口的“已归档”管理页查看。
...
- 新分屏默认 50/50，分隔线可拖动。
- 布局方向、节点关系和各层比例持久化。
- 新窗格始终在工作区根目录启动新的空 Shell，不继承源窗格的 cwd、进程、环境增量或 Agent 对话。
- 各窗格之间没有运行时关联。
...
“Agent 集成”和“已归档”属于设置窗口中的管理页面，不属于应用配置或终端配置。
...
- 在“设置 → 已归档”点击恢复，只把原工作会话放回活跃会话树；设置窗口保持打开，当前选择不变，也不创建进程。
...
设置窗口还可以包含“Agent 集成”和“已归档”等管理页面；它们是操作入口，不是第三类配置。
...
不保存终端输出、滚动历史、Agent 对话内容或凭据。v1 不使用 SQLCipher；数据库位于当前用户的应用数据目录并限制文件权限，依赖 macOS/FileVault。未来若保存凭据或原始内容，必须改用 Keychain 或字段级加密并设计迁移。
...
2. 会话树严格遵守单窗格两级、多窗格三级以及不汇总父状态的规则。
3. 横向、纵向和递归分屏可创建、调整、持久化并正确恢复。
4. 新会话与新分屏均在工作区根目录启动独立 login shell。
5. 关闭主窗口后 Agent 继续运行；重新打开窗口可继续交互。
6. `⌘Q` 确认后停止所有进程并保存恢复信息；再次启动只立即处理退出前选中的工作会话。
7. 切换到尚未实例化的会话时才创建其终端；切走后进程继续运行。
8. 归档停止全部相关进程并从会话树消失；设置窗口可以恢复或永久删除。
9. 九个首版 Agent 的兼容测试可以驱动四态变化，并在官方能力存在时捕获标题、会话标识和执行恢复。
10. 未支持 Agent 和普通命令不受 Agent 集成影响。
11. Hook 事件不包含提示词、回复、工具内容或 transcript。
12. 应用配置和终端配置只改变各自范围内的样式，不改变行为，也不读取 Ghostty 配置。
13. 签名、公证后的 GitHub Release 可以被 Sparkle 发现、验证并在用户确认后安装。
