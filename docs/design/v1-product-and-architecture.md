...
Git 工作台的分支导航区分本地分支与远程跟踪分支。本地分支默认展开并作为主要操作列表；远程分支独立分组且默认收起，搜索远程分支时自动展示匹配结果。远端默认分支的 `HEAD` 符号引用不进入列表，也不能触发 Checkout 或创建本地分支。单击分支行只更新选中状态，双击或显式选择 `Checkout` 才执行分支切换。
